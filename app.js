...
const dbURI = `mongodb://admin:admin@localhost:27017/?retryWrites=true&loadBalanced=false&serverSelectionTimeoutMS=5000&connectTimeoutMS=10000&authSource=admin&authMechanism=SCRAM-SHA-256`;
...
app.get('/edit/:name/:action',(req,res)=>{
  ...
  User.findOne({name:name})
    .then(result => {
      const checkChangePassword = req.params.action === 'changePassword';
      res.render(checkChangePassword ? 'password' : 'edit', { user: result ,title: checkChangePassword ? 'Change Password' : 'Edit-User' });
    })
    .catch(err => {
      console.log(err);
    });
...
app.post('/changePassword/:id', async (req,res)=>{
  ...
  try {
    ...
    console.log('tan tesst', newPassword);
 
    const userId = req.params.id;
    console.log('tan test', userId);
    ...
    console.log("Password Validity:", oldPassword);
